...
/**
 * 操作日志表(OperationLog)表控制层
 *
 * @author sefo
 * @since 2021-08-18 17:55:46
 */
@Api(tags = {"操作日志表(OperationLog)控制器"})
@RestController
...
public class OperationLogController {
    ...
    public OperationLog selectOne(String id) {
        return this.operationLogService.queryById(id);
    }
    
    /**
     * 分页查询
     * @param operationLog 条件对象
     * @param pageNum  页数
     * @param pageSize 每页条数
     * @return page对象
     */
    ...
    @PostMapping("/getPage")
    @ApiImplicitParams({
            @ApiImplicitParam(name="operationLog",value="条件对象",required=true,paramType="body"),
    ...
    public ResponseValue getPage(@RequestBody OperationLog operationLog, 
                                 @ApiParam(value = "页数", defaultValue = "1") int pageNum, 
                                 @ApiParam(value = "每页条数", defaultValue = "10") int pageSize) {
        return ResponseValue.success(this.operationLogService.getPage(operationLog, pageNum, pageSize));
    }
